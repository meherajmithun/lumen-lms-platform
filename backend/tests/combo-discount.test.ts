...
describe('combo discounts', () => {
  ...
  it('uses the highest eligible tier', () => {
    ...
    expect(comboDiscountFor(2, tiers)).toBe(500);
    expect(comboDiscountFor(3, tiers)).toBe(1000);
    expect(comboDiscountFor(5, tiers)).toBe(1000);
  });

  it('drops invalid tiers and sorts valid tiers', () => {
    expect(normalizeComboTiers([{ courseCount: 1, discountAmount: 50 }, { courseCount: 2, discountAmount: 500 }]))
      .toEqual([{ courseCount: 2, discountAmount: 500 }]);
  });
});
