...
export default factories.createCoreController('api::combo-offer.combo-offer', ({ strapi }) => ({
  async current() {
    ...
    return { data: { title: row.title, description: row.description, tiers: normalizeComboTiers(row.tiers), isActive: row.isActive } };
  },
  async save(ctx: ApiContext) {
    ...
    const data = { title, description, tiers, isActive: input.isActive !== false };
