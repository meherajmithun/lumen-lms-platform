...
export function ComboOfferForm({ offer }: { offer: ComboOffer }) {
  ...
  return (
    <form className="rounded-2xl border border-border/80 bg-card p-5 shadow-[var(--shadow-raised)] sm:p-6" onSubmit={(event) => {
      ...
      <div className="flex items-center justify-between gap-4"><div><h2 className="font-semibold">Combo discount</h2><p className="mt-1 text-xs text-muted-foreground">Only Content Managers can change these public discount tiers.</p></div><Switch checked={active} disabled={pending} onCheckedChange={(nextActive) => { const previous = active; setActive(nextActive); start(async () => { const result = await saveComboOfferAction({ ...offer, isActive: nextActive }); if (result.ok) { toast.success(`Combo discount ${nextActive ? 'enabled' : 'disabled'}`); router.refresh(); } else { setActive(previous); toast.error(result.error); } }); }} aria-label="Enable combo offer" /></div>
      ...
      <Button type="submit" className="mt-4" disabled={pending}>{pending ? 'Saving…' : 'Save combo offer'}</Button>
    </form>
  );
}
