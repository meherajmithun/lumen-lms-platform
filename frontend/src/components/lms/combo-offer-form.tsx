...
export function ComboOfferForm({ offer }: { offer: ComboOffer }) {
  ...
  return (
    <form className="mb-6 rounded-xl border bg-card p-5" onSubmit={(event) => {
      ...
      start(async () => {
        const result = await saveComboOfferAction({ title: String(form.get('title')), description: String(form.get('description')), tiers, isActive: active });
      ...
      <div className="mt-4 grid gap-4 sm:grid-cols-2"><div className="space-y-2"><Label htmlFor="combo-title">Offer title</Label><Input id="combo-title" name="title" defaultValue={offer.title} required /></div><div className="space-y-2"><Label htmlFor="combo-tiers">Tiers: courses:discount</Label><Textarea id="combo-tiers" name="tiers" defaultValue={tierText} rows={3} placeholder={'2:1000\n3:2500'} required /></div></div>
