...
export function ProfileEditor({ profile }: { profile: UserProfile }) {
  ...
  return (
    <form className="mt-5 grid gap-4 border-t border-border pt-5" action={(formData) => startTransition(async () => {
      ...
      <div className="grid gap-2"><Label htmlFor="profile-avatar">Profile image URL</Label><Input id="profile-avatar" name="avatarUrl" type="url" defaultValue={profile.avatarUrl} maxLength={500} placeholder="https://example.com/photo.jpg" /><p className="text-xs text-muted-foreground">Use a public http or https image URL.</p></div>
